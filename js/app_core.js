...
? `<button class="ub-login-btn" onclick="signOut()" style="${curPage==='calendar'?'display:none;':''}"><svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M10 8H2M6 5l-3 3 3 3"/><path d="M6 2h6a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1H6"/></svg>로그아웃</button>`
: `<button class="ub-login-btn" onclick="openAuthModal()" style="${curPage==='calendar'?'display:none;':''}"><svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 8h8M10 5l3 3-3 3"/><path d="M10 2H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h6"/></svg>로그인</button>`;
...
return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:0.5px solid var(--bd);">
<div style="display:flex;align-items:center;gap:10px;">
<div style="width:56px;height:56px;border-radius:8px;overflow:hidden;flex-shrink:0;background:linear-gradient(180deg,${st.bgFrom} 0%,${st.bgTo} 100%);display:flex;align-items:flex-end;justify-content:center;padding-bottom:2px;">${svgHtml}</div>
<div>
<div style="font-size:14px;font-weight:${isCur?'700':'500'};color:${unlocked?st.color:'var(--text2)'};">${st.name}</div>
<div style="font-size:11px;color:var(--text3);">${tpText}${st.reqDay?' · 연속 '+st.reqDay+'일+':''}</div>
</div>
</div>
${isCur?`<span style="font-size:12px;padding:3px 10px;border-radius:20px;background:var(--info-bg);color:var(--info);">현재</span>`:`<span style="font-size:12px;color:var(--text3);">✓ 달성</span>`}
</div>`;
} else {
return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:0.5px solid var(--bd);">
<div style="display:flex;align-items:center;gap:10px;">
<div style="width:56px;height:56px;border-radius:8px;flex-shrink:0;background:var(--bg3);border:0.5px solid var(--bd);display:flex;align-items:center;justify-content:center;">
